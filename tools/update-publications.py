...
and correctly-cased metadata, and writes two files:
  ...
  _publications-generated.md  the complete list, included by publications.qmd
  _publications-featured.md   the FEATURED papers, included above it
...
OVERRIDES = {
    ...
    # Same case: published online 2025, print issue (51(3), 322–336) came in
    # 2026 and the CV dates it 2025. The volume and pages are kept.
    "10.1111/nbu.70012": {"year": 2025},
    # Article number, not deposited by the publisher.
    "10.1186/s44410-026-00030-0": {"page": "14"},
}

# Papers set apart above the complete list, in this order. Two or three is
# the right number; they also stay in the complete list. A DOI that is not in
# the fetched records is reported and skipped rather than failing the run.
FEATURED = [
    "10.1016/j.appet.2025.108311",   # Appetite 2025 — first-author scoping review
    "10.1007/s40279-023-01871-8",    # Sports Medicine 2023 — menstrual cycle review
    "10.3390/nu13041235",            # Nutrients 2021 — first-author systematic review
]
...
def md_to_html(text):
    """The author string carries **Renard, M.** in Markdown; the entries are
    emitted as raw HTML, so convert that one construct after escaping."""
    text = html.escape(text, quote=False)
    return re.sub(r"\*\*(.+?)\*\*", r"<strong>\1</strong>", text)


def format_entry(msg):
    """One publication as raw HTML: title first, then quieter metadata, then
    the DOI link, then the BibTeX (hidden; revealed by the script at the foot
    of publications.qmd). Class names are styled in theme-rules.scss."""
    authors = md_to_html(format_authors(msg.get("author", [])))
    year = year_of(msg)
    title = html.escape(title_of(msg), quote=False)
    journal = html.escape(journal_of(msg), quote=False)
    doi = msg.get("DOI", "")
    url = f"https://doi.org/{doi}"

    where = f"<em>{journal}</em>"
    vol = msg.get("volume")
    issue = msg.get("issue")
    page = msg.get("page")
    if vol:
        where += f", <strong>{vol}</strong>"
        if issue:
            where += f"({issue})"
    if page:
        where += f", {page.replace('-', '–')}"

    bib = html.escape(bibtex(msg).rstrip(), quote=False)
    return "\n".join([
        f'<div class="pub-entry" id="pub-{bibtex_key(msg)}">',
        f'<p class="pub-title"><a href="{url}">{title}</a></p>',
        f'<p class="pub-meta">{authors} ({year}). {where}.</p>',
        f'<p class="pub-links"><a href="{url}">doi:{doi}</a></p>',
        f'<pre class="pub-bib" hidden>{bib}</pre>',
        "</div>",
    ])


def html_block(css_class, records):
    """A raw-HTML fenced block, so Pandoc passes the markup through untouched."""
    return "\n".join(
        [f"::: {{.{css_class}}}", "", "```{=html}"]
        + [format_entry(m) for m in records]
        + ["```", "", ":::", ""]
    )
...
def main():
    ...
    header = "<!-- GENERATED by tools/update-publications.py — do not edit by hand. -->\n\n"

    md_path = ROOT / "_publications-generated.md"
    md_path.write_text(header + html_block("pub-list", records))

    by_doi = {m.get("DOI", "").lower(): m for m in records}
    featured, missing = [], []
    for doi in FEATURED:
        (featured if doi.lower() in by_doi else missing).append(doi)
    feat_path = ROOT / "_publications-featured.md"
    feat_path.write_text(
        header + html_block("pub-featured", [by_doi[d.lower()] for d in featured])
    )
    ...
    print(f"\nWrote {md_path.name}, {feat_path.name} and {bib_path.name} — "
          f"{len(records)} publications, {len(featured)} featured.")
    if missing:
        print("\nFEATURED DOIs not in the fetched records (skipped):")
        for doi in missing:
            print(f"  {doi}")
